...
for(let i = 0; i < randomBoxes.length; i++) {
    if(randomBoxes[i].getBoundingClientRect().y < 100 && randomBoxes[i].getBoundingClientRect().y > -100) {
        randomBoxes[i].style.transform = `perspective(400px) rotateX(${50 - randomBoxes[i].getBoundingClientRect().y/2}deg) scale(${1-(100 - randomBoxes[i].getBoundingClientRect().y)/600})`;
